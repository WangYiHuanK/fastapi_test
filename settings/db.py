...
ASYNC_SQLALCHEMY_DATABASE_URL = "mysql+aiomysql://{}:{}@{}:{}/{}?charset=utf8mb4".format(
    configs.MYSQL_USER,
    configs.MYSQL_PASSWORD,
    configs.MYSQL_SERVER,
    configs.MYSQL_PORT,
...
# """
engine = create_engine(SQLALCHEMY_DATABASE_URL, pool_size=200, max_overflow=200, echo=False, pool_recycle=120)
...
class BaseModel(Base):
    ...
    update_time = Column(DateTime(timezone=True), default=func.now(), nullable=False, onupdate=func.now(),
                         comment='更新时间')
    is_delete = Column(Boolean, default=False, nullable=False, index=True, comment='逻辑删除')
